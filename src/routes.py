...
from src.models import Note, AddressBook, Tag
from src import db
from . import app
from validation.get_value_ab import get_value_ab
from validation.valid_for_ab import DateIsNotValid, Birthday, Phone, Name, Email, Address
...
def addressbook():
    ...
    if request.method == 'POST':
        ...
        addressbook = AddressBook(name=name, phone=phone, birthday=birthday, email=email, address=address)
        db.session.add(addressbook)
        db.session.commit()
...
def note():
    if request.method == 'POST':
        ...
        note = Note(description=description, tags=tags_obj)
        db.session.add(note)
        db.session.commit()
...
@app.route('/show_ab')
def show_ab():
    addressbooks = db.session.query(AddressBook).all()
    return render_template('pages/show_ab.html', back='/', addressbooks=addressbooks)
...
def change_ab():
    ...
    if request.method == 'POST':
        ...
        address = Address(request.form.get('address')).value
        ab = db.session.query(AddressBook).filter(AddressBook.id == int(id)).one()
...
def delete_ab():
    if request.method == 'POST':
        ...
        db.session.query(AddressBook).filter(AddressBook.id == int(id_)).delete()
        db.session.commit()
        return redirect('/show_ab')
